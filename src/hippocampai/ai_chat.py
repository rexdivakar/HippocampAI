...
class MemoryEnhancedChat:
    ...
    def _initialize_services(self) -> None:
        ...
        # Core infrastructure
        self.qdrant = QdrantManager(host=self.settings.qdrant.host, port=self.settings.qdrant.port)
        self.qdrant.create_collections()

        self.embeddings = EmbeddingService(model_name=self.settings.embedding.model)

        # Memory services
        self.memory_store = MemoryStore(
            qdrant_manager=self.qdrant, embedding_service=self.embeddings
        )
        ...
        self.deduplicator = MemoryDeduplicator(
            retriever=self.retriever, embedding_service=self.embeddings
        )

        self.updater = MemoryUpdater(
            qdrant_manager=self.qdrant, retriever=self.retriever, embedding_service=self.embeddings
        )
        ...
        self.consolidator = MemoryConsolidator(
            retriever=self.retriever, updater=self.updater, embedding_service=self.embeddings
        )
...
For factual queries (like weather, distances, calculations), provide the answer directly without extra elaboration."""
    ...
    def send_message(self, message: str, context_type: Optional[str] = None) -> str:
        """
        ...
        self.session_manager.add_message(
            session_id=self.current_session_id, role="user", content=message
        )
        ...
        self.session_manager.add_message(
            session_id=self.current_session_id, role="assistant", content=response
        )
    ...
    def _start_session(self) -> None:
        ...
        self.current_session_id = self.session_manager.start_session(user_id=self.user_id)
    ...
    def _extract_and_store_memories(self, user_message: str, assistant_message: str) -> None:
        ...
        try:
            ...
            memories = self.extractor.extract_memories(
                conversation_text=conversation_text, user_id=self.user_id
            )
    ...
    def _consolidate_memories(self) -> None:
        ...
        try:
            ...
            for cluster in clusters[:3]:  # Limit to 3 clusters per run
                self.consolidator.consolidate_cluster(cluster=cluster, user_id=self.user_id)
    ...
    def end_conversation(self) -> Optional[str]:
        ...
        summary_id = self.session_manager.end_session(session_id=self.current_session_id)
    ...
    def get_user_memories(
        ...
        try:
            ...
            memories = self.retriever.get_memories_by_filter(filters=filters, limit=limit)
            return memories
        except Exception as e:
