...
class ConversationCompactor:
    ...
    def compact_conversations(
        ...
        try:
            ...
            result.metrics.estimated_input_cost = estimate_cost(result.metrics.input_tokens, is_input=True)
            
            # Calculate average memory size before
            if memories:
                result.metrics.avg_memory_size_before = result.metrics.input_characters / len(memories)
            ...
            if len(memories) < min_memories:
                ...
                result.summary = f"Not enough memories to compact ({len(memories)} < {min_memories})"
                result.insights.append(f"Found {len(memories)} memories, need at least {min_memories} to compact")
                ...
                if len(cluster) >= 2:
                    ...
                    if summary:
                        ...
                        result.actions.append({
                            ...
                            "input_tokens": sum(estimate_tokens(m.get("text", "")) for m in cluster),
            ...
            result.metrics.output_tokens = sum(estimate_tokens(s.get("text", "")) for s in summaries)
            result.metrics.estimated_output_cost = estimate_cost(result.metrics.output_tokens, is_input=False)
            
            # Calculate average memory size after
            if summaries:
                result.metrics.avg_memory_size_after = result.metrics.output_characters / len(summaries)
            
            # 5. Calculate compression stats
            if result.metrics.input_tokens > 0:
                result.metrics.compression_ratio = 1 - (result.metrics.output_tokens / result.metrics.input_tokens)
            result.metrics.tokens_saved = result.metrics.input_tokens - result.metrics.output_tokens
            result.metrics.memories_merged = result.metrics.input_memories - result.metrics.output_memories
            ...
            if result.metrics.input_memories > 0:
                result.metrics.context_retention_score = min(1.0, 
    ...
    def _generate_insights(self, result: CompactionResult, memories: list[dict], summaries: list[dict]) -> list[str]:
        ...
        if m.compression_ratio > 0.8:
            insights.append(f"🎯 Excellent compression! Reduced token usage by {m.compression_ratio*100:.0f}%")
        elif m.compression_ratio > 0.5:
            insights.append(f"✅ Good compression achieved: {m.compression_ratio*100:.0f}% reduction")
        ...
        if m.memories_merged > 10:
            insights.append(f"📦 Consolidated {m.memories_merged} memories into {m.output_memories} summaries")
        ...
        if m.context_retention_score > 0.8:
            insights.append(f"🧠 High context retention: {m.context_retention_score*100:.0f}% of key information preserved")
        elif m.context_retention_score > 0.5:
            insights.append(f"📝 Good context retention: {m.context_retention_score*100:.0f}% of key information preserved")
        ...
        if future_savings > 0:
            insights.append(f"🚀 Projected savings: ~{future_savings:,} tokens over next 10 retrievals")
...
Summary:"""
    ...
    def _store_summaries(self, summaries: list[dict], user_id: str, session_id: Optional[str]) -> None:
